...
*****************************************************************************/
static void mapleai_manage_spaceship(struct player *pplayer)
{
...
/*************************************************************************//**
  ...
  AI settings.
*****************************************************************************/
static void mapleai_manage_taxes(struct ai_type *ait, struct player *pplayer)
{
  ...
  mapleai_calc_data(pplayer, &trade, &expenses, &income);
  ...
  while (rates[AI_RATE_TAX] <= maxrate
         && rates[AI_RATE_SCI] >= 0
         && rates[AI_RATE_LUX] >= 0) {
    bool refill_coffers = pplayer->economic.gold < mapleai_gold_reserve(pplayer);
  ...
  if (celebrate == AI_CELEBRATION_YES
    ...
    if (RATE_VALID(rate_tax_balance)) {
      ...
      if (!adv_wants_science(pplayer) || mapleai_on_war_footing(ait, pplayer)) {
  ...
  if (!adv_wants_science(pplayer)) {
    ...
    rates[AI_RATE_SCI] = MIN(maxrate, rates[AI_RATE_SCI]
                                      + RATE_REMAINS(rates));
  } else if (mapleai_on_war_footing(ait, pplayer)) {
    rates[AI_RATE_TAX] = MIN(maxrate, rates[AI_RATE_TAX]
                                      + RATE_REMAINS(rates));
    rates[AI_RATE_SCI] = MIN(maxrate, rates[AI_RATE_SCI]
                                      + RATE_REMAINS(rates));
...
*****************************************************************************/
static void mapleai_manage_government(struct ai_type *ait, struct player *pplayer)
{
  struct adv_data *adv = adv_data_get(pplayer, nullptr);
  ...
  if (adv->goal.revolution != government_of_player(pplayer)) {
    mapleai_government_change(pplayer, adv->goal.revolution); /* Change */
  }
  ...
  if (adv->goal.govt.req == A_UNSET
    ...
    TECH_LOG(ait, LOG_DEBUG, pplayer, advance_by_number(adv->goal.govt.req), 
             "mapleai_manage_government() + %d for %s",
             want,
...
void mapleai_do_first_activities(struct ai_type *ait, struct player *pplayer)
{
   ...
   * us and make mapleai_manage_units and Co act upon this information, trying
   * to eliminate the source of danger */

  TIMING_LOG(AIT_UNITS, TIMER_START);
  mapleai_manage_units(ait, pplayer);
...
{
  TIMING_LOG(AIT_ALL, TIMER_START);
  mapleai_clear_tech_wants(ait, pplayer);

  mapleai_manage_government(ait, pplayer);
  mapleai_adjust_policies(ait, pplayer);
  TIMING_LOG(AIT_TAXES, TIMER_START);
  mapleai_manage_taxes(ait, pplayer);
  TIMING_LOG(AIT_TAXES, TIMER_STOP);
  TIMING_LOG(AIT_CITIES, TIMER_START);
  mapleai_manage_cities(ait, pplayer);
  TIMING_LOG(AIT_CITIES, TIMER_STOP);
  TIMING_LOG(AIT_TECH, TIMER_START);
  mapleai_manage_tech(ait, pplayer); 
  TIMING_LOG(AIT_TECH, TIMER_STOP);
  mapleai_manage_spaceship(pplayer);
