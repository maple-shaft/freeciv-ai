                          ...
                          const int nplayers)
{
  ...
  case EFT_FORCE_CONTENT:
    v += mapleai_content_effect_value(pplayer, pcity, amount, c, FEELING_FINAL);
    break;
  case EFT_MAKE_CONTENT:
    v += mapleai_content_effect_value(pplayer, pcity, amount, c, FEELING_EFFECT);
    break;
  ...
  case VUT_SITE:
  {
    ...
    if (preq->present
      ...
      requirement_vector_iterate(&pimprove->reqs, ireq) {
        if (!mapleai_can_requirement_be_met_in_city(ireq, pplayer, pcity)) {
          return FALSE;
        }
  ...
  case VUT_SPECIALIST:
    if (preq->present) {
      ...
      requirement_vector_iterate(&(preq->source.value.specialist)->reqs,
                                 sreq) {
        if (!mapleai_can_requirement_be_met_in_city(sreq, pplayer, pcity)) {
          return FALSE;
        }
      } requirement_vector_iterate_end;
