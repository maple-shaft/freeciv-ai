...
static const struct tile_data_cache *tdc_plr_get(struct ai_type *ait,
                                                 ...
                                                 int tindex)
{
  struct ai_plr *ai = mapleai_plr_data_get(ait, plr, NULL);
                                                   ...
                                                   bool use_virt_boat)
{
  ...
  if (ferry || (use_virt_boat
    ...
    fc_assert(mapleai_is_ferry(ferry, ait));
...
BUILD_CITY:
  ...
  if (def_ai_unit_data(punit, ait)->task == AIUNIT_BUILD_CITY) {
    ...
    if (!city_can_be_built_here(nmap, ptile, punit, FALSE)) {
      mapleai_unit_new_task(ait, punit, AIUNIT_NONE, NULL);
     ...
     /* Go there */
      ...
      if (same_pos(unit_tile(punit), ptile)) {
        enum cb_error_level elevel = mapleai_do_build_city(ait, pplayer, punit);
        ...
        if (elevel != CBE_OK) {
          ...
          if (elevel == CBE_RECOVERABLE) {
             ...
             * serious recursion. */
            mapleai_auto_settler_run(ait, nmap, pplayer, punit, state);
          }
...
{
  bool caller_closes;
  struct ai_plr *ai = mapleai_plr_data_get(ait, pplayer, &caller_closes);
