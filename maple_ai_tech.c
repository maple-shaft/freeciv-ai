...
**************************************************************************/
static void mapleai_select_tech(struct ai_type *ait,
...
**************************************************************************/
static adv_want mapleai_tech_base_want(struct ai_type *ait, struct player *pplayer,
                                   struct city *pcity, struct advance *padv)
{
  ...
  adv_want orig_want = mapleai_city_want(pplayer, pcity, adv, NULL);
  adv_want final_want;
  ...
  final_want = mapleai_city_want(pplayer, pcity, adv, NULL);
...
**************************************************************************/
static void mapleai_tech_effect_values(struct ai_type *ait, struct player *pplayer)
{
  ...
  advance_iterate(padv) {
    if (research_invention_state(research_get(pplayer), advance_number(padv))
      ...
      city_list_iterate(pplayer->cities, pcity) {
        ...
        v = mapleai_tech_base_want(ait, pplayer, pcity, padv);
...
void mapleai_manage_tech(struct ai_type *ait, struct player *pplayer)
{
  ...
  mapleai_tech_effect_values(ait, pplayer);
  ...
  mapleai_select_tech(ait, pplayer, &choice, &goal);
      ...
      if (building != NULL
        ...
        requirement_vector_iterate(&building->reqs, preq) {
          if (!is_req_active(&context, NULL, preq, RPT_CERTAIN)) {
            ...
            } else if (!mapleai_can_requirement_be_met_in_city(preq, pplayer,
                                                           pcity)) {
...
**************************************************************************/
struct unit_type *mapleai_wants_role_unit(struct ai_type *ait, struct player *pplayer,
