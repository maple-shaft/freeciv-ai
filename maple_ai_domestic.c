...
/***********************************************************************//**
  ...
  stimulated.
****************************************************************************/
static void mapleai_choose_help_wonder(struct ai_type *ait,
                                   struct city *pcity,
                                   ...
                                   struct adv_data *ai)
{
  ...
  if (build_points_left(wonder_city)
    ...
    if (want > choice->want) {
       ...
       * the unit. */
      unit_type = mapleai_wants_role_unit(ait, pplayer, pcity,
...
***************************************************************************/
static void mapleai_choose_trade_route(struct ai_type *ait, struct city *pcity,
                                   struct adv_choice *choice,
                                   struct adv_data *ai)
{
  struct player *pplayer = city_owner(pcity);
                                                 ...
                                                 struct city *pcity)
{
   ...
   * mapleai_manage_cities(). The expand value is the % that the AI should
  ...
  if (worker_type != NULL
    ...
    if (worker_want > 0) {
      CITY_LOG(LOG_DEBUG, pcity,
               ...
               worker_want);
      mapleai_choose_role_unit(ait, pplayer, pcity, choice, CT_CIVILIAN,
    ...
    mapleai_wants_role_unit(ait, pplayer, pcity, UTYF_WORKERS, worker_want);
  }
  ...
  if (!game.scenario.prevent_new_cities) {
    ...
    if (founder_type
      ...
      mapleai_wants_role_unit(ait, pplayer, pcity,
                          action_id_get_role(ACTION_FOUND_CITY),
                          founder_want);
    }
  }

  {
    ...
    mapleai_choose_help_wonder(ait, pcity, cur, adv);
    ...
    mapleai_choose_trade_route(ait, pcity, cur, adv);
    choice = adv_better_choice_free(choice, cur);
  }
