...
static void mapleai_incident_war(struct player *violator, struct player *victim);
static void mapleai_incident_simple(struct player *receiver,
                                const struct player *violator,
                                const struct player *victim,
                                enum casus_belli_range scope,
                                int how_bad);
static void mapleai_incident_nuclear(struct player *receiver,
                                 const struct player *violator,
                                 const struct player *victim);
static void mapleai_incident_nuclear_not_target(struct player *receiver,
                                            const struct player *violator,
                                            const struct player *victim);
static void mapleai_incident_nuclear_self(struct player *receiver,
                                      const struct player *violator,
                                      const struct player *victim);
...
/******************************************************************//**
  ...
  in the future.
**********************************************************************/
static void mapleai_diplo_notify(struct player *pplayer, const char *text, ...)
{
...
**********************************************************************/
static int mapleai_goldequiv_tech(struct ai_type *ait,
...
**********************************************************************/
static bool mapleai_players_can_agree_on_ceasefire(struct ai_type *ait,
                                               struct player* player1,
                                               struct player* player2)
{
  return (player1->ai_common.love[player_index(player2)] > - (MAX_AI_LOVE * 4 / 10)
          && mapleai_diplomacy_get(ait, player1, player2)->countdown == -1);
}
                                   ...
                                   int tech_id, bool *is_dangerous)
{
  ...
  worth = mapleai_goldequiv_tech(ait, taker, tech_id);
  ...
  *is_dangerous = FALSE;
  ...
  players_iterate_alive(eplayer) {
    ...
    if (pplayers_allied(taker, eplayer)
      ...
      worth += mapleai_goldequiv_tech(ait, eplayer, tech_id) / 4;
    }
  } players_iterate_alive_end;
...
/******************************************************************//**
  ...
  state
**********************************************************************/
static int mapleai_goldequiv_clause(struct ai_type *ait,
                                ...
                                enum diplstate_type ds_after)
{
  ...
  struct ai_dip_intel *adip = mapleai_diplomacy_get(ait, pplayer, aplayer);
  bool is_dangerous;
  const struct civ_map *nmap = &(wld.map);

  ai = mapleai_plr_data_get(ait, pplayer, &close_here);
  ...
  case CLAUSE_CEASEFIRE:
    ...
    if ((penemy = get_allied_with_enemy_player(pplayer, aplayer))
      ...
      mapleai_diplo_notify(aplayer, _("*%s (AI)* First break alliance with %s, %s."),
    ...
    if (adip->is_allied_with_enemy
        && pclause->type != CLAUSE_CEASEFIRE) {
      mapleai_diplo_notify(aplayer, _("*%s (AI)* I would like to see you keep your "
    ...
    if (pclause->type == CLAUSE_PEACE) {
      ...
      if (pplayer->ai_common.love[player_index(aplayer)] < MAX_AI_LOVE / 10) {
        mapleai_diplo_notify(aplayer, _("*%s (AI)* I simply do not trust you with an "
                                    "alliance yet, %s."),
                         player_name(pplayer),
                         player_name(aplayer));
        worth = -BIG_NUMBER;
      ...
      if (is_ai(pplayer) && is_ai(aplayer)
          && mapleai_players_can_agree_on_ceasefire(ait, pplayer, aplayer)) {
        worth = 0;
      } else {
  ...
  if (close_here) {
    mapleai_data_phase_finished(ait, pplayer);
  }
                         ...
                         struct player *aplayer, struct treaty *ptreaty)
{
  ...
  clause_list_iterate(ptreaty->clauses, pclause) {
    ...
    total_balance +=
      mapleai_goldequiv_clause(ait, pplayer, aplayer, pclause, TRUE, ds_after);
...
/******************************************************************//**
  ...
  alter any state.
**********************************************************************/
static void mapleai_treaty_react(struct ai_type *ait,
                             struct player *pplayer,
                             struct player *aplayer,
                             struct Clause *pclause)
{
  ...
  switch (pclause->type) {
    case CLAUSE_ALLIANCE:
      if (adip->is_allied_with_ally) {
        mapleai_diplo_notify(aplayer, _("*%s (AI)* Welcome into our alliance %s!"),
                         player_name(pplayer),
                         player_name(aplayer));
      } else {
        mapleai_diplo_notify(aplayer, _("*%s (AI)* Yes, may we forever stand united, %s."),
                         player_name(pplayer),
                         player_name(aplayer));
      }
      ...
      break;
    case CLAUSE_PEACE:
      mapleai_diplo_notify(aplayer, _("*%s (AI)* Yes, peace in our time!"),
                       player_name(pplayer));
      DIPLO_LOG(ait, LOG_DIPL, pplayer, aplayer, "sign peace treaty");
      break;
    case CLAUSE_CEASEFIRE:
      mapleai_diplo_notify(aplayer, _("*%s (AI)* Agreed. No more hostilities, %s."),
                       player_name(pplayer),
                       player_name(aplayer));
                         ...
                         struct player *aplayer, struct treaty *ptreaty)
{
  ...
  clause_list_iterate(ptreaty->clauses, pclause) {
    ...
    gift = (gift && (balance >= 0));
    mapleai_treaty_react(ait, pplayer, aplayer, pclause);
    if (is_pact_clause(pclause->type)
        && mapleai_diplomacy_get(ait, pplayer, aplayer)->countdown != -1) {
...
**********************************************************************/
static int mapleai_war_desire(struct ai_type *ait, struct player *pplayer,
                          struct player *target)
{
  struct ai_plr *ai = mapleai_plr_data_get(ait, pplayer, nullptr);
  ...
  mapleai_diplomacy_get(ait, pplayer, target)->distance = distance;
...
**********************************************************************/
static void mapleai_diplomacy_suggest(struct player *pplayer,
                                  struct player *aplayer,
                                 ...
                                 struct player *aplayer)
{
  ...
  if (is_ai(pplayer)
    ...
    if (has_handicap(pplayer, H_CEASEFIRE)) {
      ...
      if (mapleai_goldequiv_clause(ait, pplayer, aplayer, &clause,
  ...
  if (wants_ceasefire) {
    ...
    mapleai_diplomacy_suggest(pplayer, aplayer, CLAUSE_CEASEFIRE, FALSE, 0);
  } else {
    mapleai_diplo_notify(aplayer,
...
void mapleai_diplomacy_begin_new_phase(struct ai_type *ait, struct player *pplayer)
{
  ...
  players_iterate_alive(aplayer) {
    ...
    war_desire[player_index(aplayer)] = mapleai_war_desire(ait, pplayer, aplayer);
  ...
  advance_index_iterate_max(A_FIRST, tech, ac) {
    ...
    advance_index_iterate_max(A_FIRST, tech2, ac) {
      ...
      if ((diff > 0 && player1->economic.gold >= diff)
          ...
          || diff == 0) {
        mapleai_diplomacy_suggest(player1, player2, CLAUSE_ADVANCE, FALSE, tech2);
        mapleai_diplomacy_suggest(player1, player2, CLAUSE_ADVANCE, TRUE, tech);
        if (diff > 0) {
          mapleai_diplomacy_suggest(player1, player2, CLAUSE_GOLD, FALSE, diff);
        } else if (diff < 0) {
          mapleai_diplomacy_suggest(player1, player2, CLAUSE_GOLD, TRUE, -diff);
        }
        return;
      }
    } advance_index_iterate_max_end;
...
**********************************************************************/
static void mapleai_share(struct ai_type *ait, struct player *pplayer,
                      struct player *aplayer)
{
  struct research *presearch = research_get(pplayer);
    ...
    advance_index_iterate(A_FIRST, idx) {
      if (research_invention_state(presearch, idx) != TECH_KNOWN
          ...
          && research_invention_gettable(presearch, idx,
                                         game.info.tech_trade_allow_holes)) {
       mapleai_diplomacy_suggest(pplayer, aplayer, CLAUSE_ADVANCE, TRUE, idx);
      } else if (research_invention_state(presearch, idx) == TECH_KNOWN
                 && research_invention_state(aresearch, idx) != TECH_KNOWN
                 && research_invention_gettable(aresearch, idx,
                                                game.info.tech_trade_allow_holes)) {
        mapleai_diplomacy_suggest(pplayer, aplayer, CLAUSE_ADVANCE, FALSE, idx);
      }
  ...
  if (!gives_vision
    ...
    mapleai_diplomacy_suggest(pplayer, aplayer, CLAUSE_VISION, FALSE, 0);
    gives_vision = TRUE;
  }
  if (gives_vision
    ...
    mapleai_diplomacy_suggest(pplayer, aplayer, CLAUSE_VISION, TRUE, 0);
  }

  if (!player_has_embassy(pplayer, aplayer)) {
    mapleai_diplomacy_suggest(pplayer, aplayer, CLAUSE_EMBASSY, TRUE, 0);
  }
  if (!player_has_embassy(aplayer, pplayer)) {
    mapleai_diplomacy_suggest(pplayer, aplayer, CLAUSE_EMBASSY, FALSE, 0);
  }
...
**********************************************************************/
static void mapleai_declare_war(struct ai_type *ait, struct player *pplayer,
                            struct player *target)
{
  struct ai_dip_intel *adip = mapleai_diplomacy_get(ait, pplayer, target);
...
**********************************************************************/
static void mapleai_go_to_war(struct ai_type *ait, struct player *pplayer,
                          struct player *target, enum war_reason reason)
{
  ...
  switch (reason) {
  case DAI_WR_SPACE:
    mapleai_diplo_notify(target, _("*%s (AI)* Space will never be yours."),
                     player_name(pplayer));
    adip->countdown = -10;
    break;
  case DAI_WR_BEHAVIOR:
    mapleai_diplo_notify(target,
    ...
    break;
  case DAI_WR_NONE:
    mapleai_diplo_notify(target, _("*%s (AI)* Peace in ... some other time."),
                     player_name(pplayer));
    adip->countdown = -10;
    break;
  case DAI_WR_HATRED:
  ...
  mapleai_declare_war(ait, pplayer, target);
}
...
void mapleai_revolution_start(struct ai_type *ait, struct player *pplayer)
{
  ...
  if (data->diplomacy.war_target != nullptr) {
    ...
    mapleai_declare_war(ait, pplayer, data->diplomacy.war_target);
...
void mapleai_diplomacy_actions(struct ai_type *ait, struct player *pplayer)
{
  ...
  if (need_targets && target && most_hatred < war_threshold
      && mapleai_diplomacy_get(ait, pplayer, target)->countdown == -1) {
    ...
    if (!players_on_same_team(pplayer, aplayer)) {
      ...
      if (adip->countdown > 0) {
        ...
        if (!WAR(pplayer, aplayer)) {
          ...
          mapleai_go_to_war(ait, pplayer, aplayer, adip->war_reason);
        }
  ...
  players_iterate_alive(aplayer) {
    if (get_player_bonus(aplayer, EFT_NO_DIPLOMACY) <= 0
      ...
      switch (ds) {
      case DS_TEAM:
        mapleai_share(ait, pplayer, aplayer);
        break;
      case DS_ALLIANCE:
      ...
      case DS_PEACE:
        ...
        if (adip->at_war_with_ally
            ...
            || mapleai_goldequiv_clause(ait, pplayer, aplayer, &clause,
                                    FALSE, DS_ALLIANCE) < 0) {
          break;
        }
        clear_old_treaty(pplayer, aplayer);
        mapleai_diplomacy_suggest(pplayer, aplayer, CLAUSE_ALLIANCE, FALSE, 0);
      ...
      case DS_CEASEFIRE:
        ...
        clear_old_treaty(pplayer, aplayer);
        mapleai_diplomacy_suggest(pplayer, aplayer, CLAUSE_PEACE, FALSE, 0);
...
{
  players_iterate(plr) {
    if (mapleai_diplomacy_get(ait, pplayer, plr)->countdown >= 0) {
      return TRUE;
    }
  } players_iterate_end;
                  ...
                  struct player *violator, struct player *victim)
{
  ...
  case INCIDENT_ACTION:
    ...
    case ACTRES_NUKE_UNITS:
      if (receiver == victim) {
        /* Tell the victim */
        mapleai_incident_nuclear(receiver, violator, victim);
      } else if (violator == victim) {
        mapleai_incident_nuclear_self(receiver, violator, victim);
      } else {
        mapleai_incident_nuclear_not_target(receiver, violator, victim);
      }
      break;
    case ACTRES_ESTABLISH_EMBASSY:
    case ACTRES_SPY_INVESTIGATE_CITY:
      /* Snoping */
      mapleai_incident_simple(receiver, violator, victim, scope, 2);
      break;
    ...
    case ACTRES_STEAL_MAPS:
      /* Theft */
      mapleai_incident_simple(receiver, violator, victim, scope, 5);
      break;
    case ACTRES_EXPEL_UNIT:
      /* Unit position loss */
      mapleai_incident_simple(receiver, violator, victim, scope, 1);
      break;
    case ACTRES_SPY_SABOTAGE_UNIT:
      /* Unit weakening */
      mapleai_incident_simple(receiver, violator, victim, scope, 3);
      break;
    ...
    case ACTRES_CONQUER_CITY:
      /* City loss */
      mapleai_incident_simple(receiver, violator, victim, scope, 10);
      break;
    ...
    case ACTRES_FOUND_CITY:
      /* Terrain loss */
      mapleai_incident_simple(receiver, violator, victim, scope, 4);
      break;
  ...
  case INCIDENT_WAR:
    if (receiver == victim) {
      mapleai_incident_war(violator, victim);
    }
    break;
  case INCIDENT_LAST:
