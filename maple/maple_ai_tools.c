                                ...
                                enum override_bool *result)
{
  ...
  adip = mapleai_diplomacy_get(ait, plr1, plr2);
...
/**********************************************************************//**
  ...
  needed.
**************************************************************************/
static bool mapleai_gothere_bodyguard(struct ai_type *ait,
                                  struct unit *punit, struct tile *dest_tile)
{
  ...
  if (dcity && POTENTIALLY_HOSTILE_PLAYER(ait, pplayer, city_owner(dcity))) {
    ...
    struct unit_type *d_type = mapleai_choose_defender_versus(dcity, punit);
...
/**********************************************************************//**
  ...
  units (i.e. planes or ships), use mapleai_unit_goto() instead.
                 ...
                 struct unit *punit, struct tile *dest_tile)
{
  ...
  bg_needed = mapleai_gothere_bodyguard(ait, punit, dest_tile);
  ...
  if (goto_is_sane(punit, dest_tile) && punit->moves_left > 0) {
    ...
    if (!mapleai_unit_goto(ait, punit, dest_tile)) {
      /* Died */
      return FALSE;
    }
                               ...
                               struct pf_parameter *parameter)
{
  ...
  if (path) {
    mapleai_log_path(punit, path, parameter);
...
**************************************************************************/
void mapleai_fill_unit_param(struct ai_type *ait, struct pf_parameter *parameter,
...
bool mapleai_unit_goto(struct ai_type *ait, struct unit *punit, struct tile *ptile)
{
  ...
  UNIT_LOG(LOG_DEBUG, punit, "mapleai_unit_goto() to %d,%d", TILE_XY(ptile));
  mapleai_fill_unit_param(ait, &parameter, &risk_cost, punit, ptile);

  return mapleai_unit_goto_constrained(ait, punit, ptile, &parameter);
}
                           ...
                           enum adv_unit_task task, struct tile *ptile)
{
  ...
  switch (task) {
   case AUT_AUTO_WORKER:
     mapleai_unit_new_task(ait, punit, AIUNIT_AUTO_WORKER, ptile);
     break;
   case AUT_BUILD_CITY:
     mapleai_unit_new_task(ait, punit, AIUNIT_BUILD_CITY, ptile);
     break;
   case AUT_NONE:
     mapleai_unit_new_task(ait, punit, AIUNIT_NONE, ptile);
     break;
  }
}
                       ...
                       enum ai_unit_task task, struct tile *ptile)
{
  ...
  UNIT_LOG(LOG_DEBUG, punit, "changing task from %s to %s",
           ...
           mapleai_unit_task_rule_name(task));
  ...
  if (unit_data->task == AIUNIT_NONE && bodyguard) {
    mapleai_unit_new_task(ait, bodyguard, AIUNIT_NONE, nullptr);
  }
  ...
  if (unit_data->task == AIUNIT_HUNTER) {
    ...
    unit_list_iterate(unit_tile(punit)->units, missile) {
      if (unit_owner(missile) == unit_owner(punit)
        ...
        mapleai_unit_new_task(ait, missile, AIUNIT_ESCORT, unit_tile(target));
...
/**********************************************************************//**
  ...
  bodyguard has not. This is an mapleai_unit_*() auxiliary function, do not use
  elsewhere.
**************************************************************************/
static void mapleai_unit_bodyguard_move(struct ai_type *ait,
                                    struct unit *bodyguard, struct tile *ptile)
{
  ...
  (void) mapleai_unit_move(ait, bodyguard, ptile);
}
                     ...
                     struct tile *ptile)
{
  ...
  if (alive && same_pos(ptile, unit_tile(punit))
    ...
    mapleai_unit_bodyguard_move(ait, bodyguard, ptile);
                             ...
                             struct tile *ptile, struct pf_path *path, int step)
{
  if (step == path->length - 1) {
    (void) mapleai_unit_attack(ait, punit, ptile);
  } else {
    (void) mapleai_unit_move(ait, punit, ptile);
  }
}
...
/**********************************************************************//**
  Calls mapleai_wants_role_unit() to choose the best unit with the given role
...
bool mapleai_choose_role_unit(struct ai_type *ait, struct player *pplayer,
                          ...
                          bool need_boat)
{
  struct unit_type *iunit = mapleai_wants_role_unit(ait, pplayer, pcity, role, want);
