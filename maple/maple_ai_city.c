...
static void mapleai_city_sell_noncritical(struct city *pcity,
                                      bool redundant_only);
                                             ...
                                             adv_want building_want)
{
   ...
   * and might need adjustment. See also mapleai_tech_effect_values()
   */
...
**************************************************************************/
static void mapleai_barbarian_choose_build(struct player *pplayer,
                                       struct city *pcity,
...
/**********************************************************************//**
  ...
  improvement, etc.
**************************************************************************/
static void mapleai_city_choose_build(struct ai_type *ait,
                                  struct player *pplayer,
                                  struct city *pcity)
{
  ...
  if (city_data->choice.want == 0) {
    ...
    if (best_role_unit(pcity, action_id_get_role(ACTION_TRADE_ROUTE))) {
      ...
      if (our_def == 0
          && mapleai_process_defender_want(ait, nmap, pplayer, pcity, 1,
                                       &(city_data->choice), 0)) {
        ...
        city_data->choice.value.utype
          = mapleai_role_utype_for_terrain_class(pcity, UTYF_WORKERS, TC_LAND);
        city_data->choice.type = CT_CIVILIAN;
...
**************************************************************************/
static void mapleai_upgrade_units(struct city *pcity, int limit, bool military)
{
  ...
  mapleai_calc_data(pplayer, nullptr, &expenses, nullptr);
...
**************************************************************************/
static void mapleai_spend_gold(struct ai_type *ait, struct player *pplayer)
{
  struct adv_choice bestchoice;
  int cached_limit = mapleai_gold_reserve(pplayer);
  int expenses;
  bool war_footing = mapleai_on_war_footing(ait, pplayer);
    ...
    if (city_data->urgency > 1) {
      if (bestchoice.type == CT_BUILDING
       && is_wonder(bestchoice.value.building)) {
        ...
        mapleai_upgrade_units(pcity, upgrade_limit, TRUE);
      }
    }
  ...
  if (!war_footing) {
    ...
    city_list_iterate(pplayer->cities, pcity) {
      mapleai_upgrade_units(pcity, cached_limit, FALSE);
    } city_list_iterate_end;
  }
                                             ...
                                             struct city *pcity)
{
  ...
  struct ai_city *city_data = def_ai_city_data(pcity, ait);
  struct mapleai_private_data *private = (struct mapleai_private_data *)ait->private;
  const struct civ_map *nmap = &(wld.map);
  ...
  utype = mapleai_role_utype_for_terrain_class(pcity, UTYF_WORKERS, TC_LAND);
  ...
  city_data->worker_type
    = mapleai_role_utype_for_terrain_class(pcity, UTYF_WORKERS,
...
void mapleai_manage_cities(struct ai_type *ait, struct player *pplayer)
{
  ...
  city_list_iterate(pplayer->cities, pcity) {
    ...
    mapleai_city_sell_noncritical(pcity, TRUE);
    sync_cities();
  } city_list_iterate_end;
  if (pplayer->economic.tax >= 30 /* Otherwise expect it to increase tax */
    ...
    while (player_get_expected_income(pplayer) < -(pplayer->economic.gold)
           && i < count) {
      mapleai_city_sell_noncritical(sellers[i++], FALSE);
    }
  }
  ...
  city_list_iterate(pplayer->cities, pcity) {
    ...
    if (mapleai_on_war_footing(ait, pplayer) && city_data->choice.want > 0) {
  ...
  mapleai_auto_settler_reset(ait, pplayer);

  city_list_iterate(pplayer->cities, pcity) {
    mapleai_city_choose_build(ait, pplayer, pcity);

    /* Initialize for next turn */
    def_ai_city_data(pcity, ait)->choice.want = -1;
  } city_list_iterate_end;

  mapleai_spend_gold(ait, pplayer);
}
                          ...
                          struct city *pcity, struct impr_type *pimprove)
{
  ...
  city_range_iterate(pcity, pplayer->cities,
    ...
    final_want += mapleai_city_want(pplayer, acity, adv, pimprove)
                                                ...
                                                const bool already)
{
  ...
  effect_list_iterate(get_req_source_effects(&source), peffect) {
    ...
    requirement_vector_iterate(&peffect->reqs, preq) {
      ...
      if (!is_req_active(&effect_ctxt, nullptr, preq, RPT_POSSIBLE)) {
        ...
        if (VUT_ADVANCE == preq->source.kind && preq->present) {
          ...
          tech_vector_append(&needed_techs, preq->source.value.advance);
        } else if (!mapleai_can_requirement_be_met_in_city(preq, pplayer, pcity)) {
    ...
    if ((active || n_needed_techs) && !impossible_to_get) {
      adv_want v1 = mapleai_effect_value(pplayer, ai, pcity, capital,
...
static bool should_force_recalc(struct city *pcity)
{
  return city_built_last_turn(pcity)
      || (VUT_IMPROVEMENT == pcity->production.kind
       ...
       && !mapleai_can_city_build_improvement_later
...
void mapleai_build_adv_init(struct ai_type *ait, struct player *pplayer)
{
  ...
  city_list_iterate(pplayer->cities, pcity) {
    def_ai_city_data(pcity, ait)->worth = mapleai_city_want(pplayer, pcity, ai,
                                                        nullptr);
  } city_list_iterate_end;
}
...
**************************************************************************/
void mapleai_consider_wonder_city(struct ai_type *ait, struct city *pcity,
                              bool *result)
{
...
**************************************************************************/
static bool mapleai_cant_help_req(const struct req_context *context,
                                          ...
                                          const struct impr_type *pimprove)
{
  ...
  if (!mapleai_can_player_build_improvement_later(city_owner(pcity),
                                              pimprove)) {
    return FALSE;
  }
  ...
  requirement_vector_iterate(&pimprove->reqs, preq) {
    if (mapleai_cant_help_req(&city_ctxt, preq)) {
      return FALSE;
    }
  } requirement_vector_iterate_end;
...
**************************************************************************/
bool
mapleai_can_player_build_improvement_later(const struct player *p,
                                       const struct impr_type *pimprove)
{
  ...
  requirement_vector_iterate(&pimprove->reqs, preq) {
    if (preq->range >= REQ_RANGE_PLAYER
        && mapleai_cant_help_req(&context, preq)) {
      return FALSE;
    }
  } requirement_vector_iterate_end;
