...
static bool mapleai_find_boat_for_unit(struct ai_type *ait, struct unit *punit);
static bool mapleai_caravan_can_trade_cities_diff_cont(struct player *pplayer,
                                                   struct unit *punit);
static void mapleai_manage_caravan(struct ai_type *ait, struct player *pplayer,
                               struct unit *punit);
static void mapleai_manage_barbarian_leader(struct ai_type *ait,
                                        struct player *pplayer,
                                        struct unit *leader);

static void mapleai_military_findjob(struct ai_type *ait, const struct civ_map *nmap,
                                 struct player *pplayer, struct unit *punit);
static void mapleai_military_defend(struct ai_type *ait, struct player *pplayer,
                                struct unit *punit);
static void mapleai_military_attack(struct ai_type *ait, struct player *pplayer,
                                struct unit *punit);
...
**************************************************************************/
static void mapleai_airlift(struct ai_type *ait, struct player *pplayer)
{
...
**************************************************************************/
static int mapleai_rampage_want(struct unit *punit, struct tile *ptile)
{
                                           ...
                                           int thresh_adj, int thresh_move)
{
  ...
  pf_map_move_costs_iterate(tgt_map, iter_tile, move_cost, FALSE) {
    ...
    want = mapleai_rampage_want(punit, iter_tile);
...
/**********************************************************************//**
  ...
  For example, if unit is a bodyguard on duty, it should call
    mapleai_military_rampage(punit, 100, RAMPAGE_FREE_CITY_OR_BETTER)
...
**************************************************************************/
static void mapleai_military_bodyguard(struct ai_type *ait, struct player *pplayer,
                                   struct unit *punit)
{
  ...
  if (aunit && unit_owner(aunit) == unit_owner(punit)) {
    ...
    mapleai_unit_new_task(ait, punit, AIUNIT_NONE, nullptr);
    return;
  }
  ...
  if (same_pos(unit_tile(punit), ptile)) {
    ...
    if (goto_is_sane(punit, ptile)) {
      ...
      if (!mapleai_gothere(ait, pplayer, punit, ptile)) {
        /* We died */
        return;
      }
    } else {
  ...
  if (mapleai_military_rampage(punit, BODYGUARD_RAMPAGE_THRESHOLD,
                           RAMPAGE_FREE_CITY_OR_BETTER)
  ...
  *acity = nullptr;
  ...
  pf_map_move_costs_iterate(pfm, ptile, move_cost, TRUE) {
    ...
    unit_list_iterate(ptile->units, buddy) {
      ...
      if (!mapleai_can_unit_type_follow_unit_type(ptype, buddy_type, ait)
                                 ...
                                 struct player *pplayer, struct unit *punit)
{
  ...
  if (aiguard_has_charge(ait, punit)
    ...
    if ((aunit && (aiguard_has_guard(ait, aunit) || aiguard_wanted(ait, aunit))
      ...
      mapleai_unit_new_task(ait, punit, AIUNIT_NONE, nullptr); /* Nope! */
    }
  }
  ...
  if ((unit_data->task == AIUNIT_RECOVER
    ...
    mapleai_unit_new_task(ait, punit, AIUNIT_RECOVER, nullptr);
    return;
  }
  ...
  if (unit_role_defender(unit_type_get(punit))) {
    ...
    if (acity) {
      mapleai_unit_new_task(ait, punit, AIUNIT_ESCORT, acity->tile);
      aiguard_assign_guard_city(ait, acity, punit);
      BODYGUARD_LOG(ait, LOG_DEBUG, punit, "going to defend city");
    } else if (aunit) {
      mapleai_unit_new_task(ait, punit, AIUNIT_ESCORT, unit_tile(aunit));
                                ...
                                struct unit *punit)
{
  ...
  if (mapleai_military_rampage(punit, BODYGUARD_RAMPAGE_THRESHOLD * 5,
                           RAMPAGE_FREE_CITY_OR_BETTER)) {
    /* ... We survived */
    if (pcity) {
      ...
      if (same_pos(unit_tile(punit), pcity->tile)) {
        ...
        (void) mapleai_gothere(ait, pplayer, punit, pcity->tile);
      }
    } else {
                                ...
                                const struct unit_type **pboattype, int *pmove_time)
{
  ...
  *pdest_tile = punit_tile;
  ...
  if (pcity != nullptr && (punit->id == 0 || pcity->id == punit->homecity)) {
    ...
    unhap = mapleai_assess_military_unhappiness(nmap, pcity);
  }
  ...
  players_iterate(aplayer) {
    ...
    city_list_iterate(aplayer->cities, acity) {
      ...
      if (1 < move_time) {
        struct unit_type *def_type = mapleai_choose_defender_versus(acity, punit);
...
**************************************************************************/
static void mapleai_military_attack_barbarian(struct ai_type *ait,
                                          struct player *pplayer,
                                          struct unit *punit)
{
  struct city *pc;
  ...
  if ((pc = find_closest_city(unit_tile(punit), nullptr, pplayer, FALSE,
    ...
    if (can_unit_exist_at_tile(&(wld.map), punit, unit_tile(punit))) {
      UNIT_LOG(LOG_DEBUG, punit, "Barbarian heading to conquer %s",
               city_name_get(pc));
      (void) mapleai_gothere(ait, pplayer, punit, pc->tile);
    } else {
                                ...
                                struct unit *punit)
{
   ...
   * NB: We do not need to repeat mapleai_military_rampage(), it is repeats itself
   * until it runs out of targets. */
  /* FIXME: 1. mapleai_military_rampage() never checks if it should defend newly
   * conquered cities.
   * FIXME: 2. would be more convenient if it returned FALSE if we run out
   * of moves too.*/
  if (!mapleai_military_rampage(punit, RAMPAGE_ANYTHING, RAMPAGE_ANYTHING)) {
    return; /* We died */
  }
    ...
    if (!same_pos(unit_tile(punit), dest_tile)) {
      ...
      if (is_tiles_adjacent(unit_tile(punit), dest_tile)) {
        ...
        UNIT_LOG(LOG_DEBUG, punit, "mil att bash -> (%d, %d)",
                 TILE_XY(dest_tile));
        if (!mapleai_unit_attack(ait, punit, dest_tile)) {
          /* Died */
          pf_path_destroy(path);
          return;
        }
        ...
        (void) mapleai_military_rampage(punit,
  ...
  if ((ferryboat <= 0)) {
    ...
    if (current_city == nullptr) {
      ...
      if (city_near != nullptr) {
        alive = mapleai_unit_goto(ait, punit, city_near->tile);
      }
    }
  } else {
...
**************************************************************************/
static void mapleai_caravan_goto(struct ai_type *ait, struct player *pplayer,
                             struct unit *punit,
                             ...
                             bool required_boat, bool request_boat)
{
  ...
  if (!same_pos(dest_city->tile, unit_tile(punit))
    ...
    log_base(LOG_CARAVAN, "%s %s[%d](%d,%d) task %s going to %s in %s %s",
             ...
             mapleai_unit_task_rule_name(unit_data->task),
    ...
    if (required_boat) {
      ...
      if (request_boat) {
        /* Try to find new boat */
        alive = mapleai_find_boat_for_unit(ait, punit);
      } else {
      ...
      if (alive)  {
        ...
        alive = mapleai_gothere(ait, pplayer, punit, dest_city->tile);
      }
    } else {
      /* To trade without boat */
      alive = mapleai_unit_goto(ait, punit, dest_city->tile);
    }
  }
...
**************************************************************************/
static bool mapleai_is_unit_tired_waiting_boat(struct ai_type *ait,
                                           struct unit *punit)
{
...
static bool search_homecity_for_caravan(struct ai_type *ait, struct unit *punit)
{
  ...
  if (nearest != nullptr) {
    alive = mapleai_unit_goto(ait, punit, nearest->tile);
    if (alive && same_pos(unit_tile(punit), nearest->tile)) {
      mapleai_unit_make_homecity(punit, nearest);
    }
  }
                               ...
                               struct unit *punit)
{
  ...
  if ((unit_data->task == AIUNIT_TRADE
    ...
    if ((city_dest == nullptr)
      ...
      if (expect_boats && mapleai_is_unit_tired_waiting_boat(ait, punit)) {
        aiferry_clear_boat(ait, punit);
        mapleai_unit_new_task(ait, punit, AIUNIT_NONE, nullptr);
  ...
  if (unit_data->task == AIUNIT_NONE) {
    ...
    if (mapleai_caravan_can_trade_cities_diff_cont(pplayer, punit)) {
    ...
    if (result.dest != nullptr) {
      ...
      mapleai_unit_new_task(ait, punit,
  ...
  if (dest != nullptr) {
    mapleai_caravan_goto(ait, pplayer, punit, dest, help_wonder,
...
**************************************************************************/
static void mapleai_manage_hitpoint_recovery(struct ai_type *ait,
                                         struct unit *punit)
{
  struct player *pplayer = unit_owner(punit);
  ...
  if (pcity != nullptr) {
    ...
    if (mapleai_military_rampage(punit, RAMPAGE_ANYTHING,
                             RAMPAGE_FREE_CITY_OR_BETTER)) {
    ...
    if (!mapleai_military_rampage(punit, RAMPAGE_FREE_CITY_OR_BETTER,
                              RAMPAGE_FREE_CITY_OR_BETTER)) {
      return; /* Oops, we died */
    }
    ...
    if (safe) {
      UNIT_LOG(LOGLEVEL_RECOVERY, punit, "going to %s to recover",
               city_name_get(safe));
      if (!mapleai_unit_goto(ait, punit, safe->tile)) {
      ...
      mapleai_unit_new_task(ait, punit, AIUNIT_NONE, nullptr);
      mapleai_military_attack(ait, pplayer, punit);
      return;
    }
  }
                         ...
                         struct player *pplayer, struct unit *punit)
{
   ...
   * to attack. */
  mapleai_military_findjob(ait, nmap, pplayer, punit);
  ...
  case AIUNIT_ESCORT:
    TIMING_LOG(AIT_BODYGUARD, TIMER_START);
    mapleai_military_bodyguard(ait, pplayer, punit);
    TIMING_LOG(AIT_BODYGUARD, TIMER_STOP);
    break;
  case AIUNIT_EXPLORE:
    switch (manage_auto_explorer(punit)) {
  ...
  case AIUNIT_RECOVER:
    ...
    mapleai_manage_hitpoint_recovery(ait, punit);
...
/**********************************************************************//**
  Manages settlers.
**************************************************************************/
static void mapleai_manage_settler(struct ai_type *ait, struct player *pplayer,
                               struct unit *punit)
{
  struct unit_ai *unit_data = def_ai_unit_data(punit, ait);
                     ...
                     struct unit *punit)
{
  ...
  if (utype_has_flag(ptype, UTYF_DIPLOMAT)) {
    ...
    mapleai_manage_airunit(ait, pplayer, punit);
    ...
    mapleai_manage_military(ait, nmap, pplayer, punit);
    ...
    default:
      ...
      mapleai_unit_new_task(ait, punit, AIUNIT_DEFEND_HOME, nullptr);
      mapleai_military_defend(ait, pplayer, punit);
      break;
    };
...
**************************************************************************/
static void mapleai_set_defenders(struct ai_type *ait, struct player *pplayer)
{
  city_list_iterate(pplayer->cities, pcity) {
    ...
    while (!enough
      ...
      if (best == nullptr) {
        ...
        if ((martless_unhappy < mart_each * count
          ...
          mapleai_unit_new_task(ait, best, AIUNIT_DEFEND_HOME, pcity->tile);
          count++;
        }
      }
    }
...
void mapleai_manage_units(struct ai_type *ait, struct player *pplayer)
{
  ...
  mapleai_set_defenders(ait, pplayer);
  ...
  unit_list_iterate_safe(pplayer->units, punit) {
    if ((!unit_transported(punit)
      ...
      mapleai_manage_unit(ait, pplayer, punit);
    }
                                                ...
                                                const struct city *pcity)
{
  ...
  requirement_vector_iterate(&putype->build_reqs, preq) {
    ...
    if (!mapleai_can_requirement_be_met_in_city(preq,
                                        ...
                                        struct unit *leader)
{
  ...
  if (is_boss_of_boat(ait, leader)) {
    ...
    unit_list_iterate(leader_tile->units, warrior) {
      if (!unit_has_type_role(warrior, L_BARBARIAN_LEADER)
        ...
        mapleai_manage_unit(ait, pplayer, warrior);
  ...
  do {
    ...
    alive = mapleai_unit_goto(ait, leader, safest_tile);
    if (alive) {
