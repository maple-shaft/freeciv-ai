...
static adv_want mapleai_unit_attack_desirability(struct ai_type *ait,
                                             const struct unit_type *punittype);
static adv_want mapleai_unit_defense_desirability(struct ai_type *ait,
                                              const struct unit_type *punittype);
...
**************************************************************************/
static struct unit_type *mapleai_choose_attacker(struct ai_type *ait,
                                             const struct civ_map *nmap,
                                             struct city *pcity,
                                             ...
                                             bool allow_gold_upkeep)
{
  ...
  simple_ai_unit_type_iterate(putype) {
    ...
    cur = mapleai_unit_attack_desirability(ait, putype);
...
**************************************************************************/
static struct unit_type *mapleai_choose_bodyguard(struct ai_type *ait,
                                              const struct civ_map *nmap,
                                              struct city *pcity,
                                              enum terrain_class tc,
                                              ...
                                              bool allow_gold_upkeep)
{
  ...
  simple_ai_unit_type_iterate(putype) {
    ...
    /* Now find best */
    if (can_city_build_unit_now(nmap, pcity, putype)) {
      const adv_want desire = mapleai_unit_defense_desirability(ait, putype);
...
**************************************************************************/
static void mapleai_reevaluate_building(struct city *pcity, adv_want *value, 
                                  ...
                                  player_unit_list_getter ul_cb)
{
  ...
  players_iterate(aplayer) {
    ...
    unit_list_iterate(units, punit) {
      ...
      (void) mapleai_wants_defender_against(ait, nmap, pplayer, pcity, utype,
      ...
      if (unit_can_do_action(punit, ACTION_NUKE)
        ...
        defender = mapleai_find_source_building(pcity, EFT_NUKE_PROOF, utype);
        if (defender != B_LAST) {
        ...
        defender = mapleai_find_source_building(pcity, EFT_DEFEND_BONUS, utype);
        if (defender != B_LAST) {
  ...
  for (i = 0; i < B_LAST; i++) {
    if (0 < danger_reduced[i]) {
      mapleai_reevaluate_building(pcity, &pcity->server.adv->building_want[i],
                               ...
                               struct adv_choice *choice, adv_want extra_want)
{
  ...
  simple_ai_unit_type_iterate(punittype) {
    ...
    desire = mapleai_unit_defense_desirability(ait, punittype);
                                  ...
                                  const struct unit_type *boattype)
{
  ...
  simple_ai_unit_type_iterate(punittype) {
    if (mapleai_can_unit_type_follow_unit_type(punittype, orig_utype, ait)
                                              ...
                                              struct adv_choice *choice)
{
  ...
  if (NULL != acity) {
    ...
    def_type = mapleai_choose_defender_versus(acity, myunit);
  ...
  if (best_choice->want > choice->want) {
    ...
    if (NULL != ferry_map && !ferryboat) { /* need a new ferry */
      ...
      fc_assert_ret_val(unit_class_get(myunit)->adv.sea_move != MOVE_FULL, choice);
      if (mapleai_choose_role_unit(ait, pplayer, pcity, choice, CT_ATTACKER,
			       L_FERRYBOAT, choice->want, TRUE)
	  && mapleai_is_ferry_type(choice->value.utype, ait)) {
#ifdef FREECIV_DEBUG
        struct ai_plr *ai = mapleai_plr_data_get(ait, pplayer, NULL);
...
**************************************************************************/
static void mapleai_unit_consider_bodyguard(struct ai_type *ait,
                                        const struct civ_map *nmap,
                                        struct city *pcity,
                                  ...
                                  struct adv_choice *choice)
{
  ...
  if ((id = mapleai_find_source_building(pcity, EFT_VETERAN_BUILD,
                                                 ...
                                                 player_unit_list_getter ul_cb)
{
  ...
  mapleai_choose_diplomat_defensive(ait, pplayer, pcity, choice, our_def);
  ...
  if (martial_need
    ...
    martial_value = mapleai_content_effect_value(pplayer, pcity,
  ...
  if (city_data->danger != 0 || martial_value > 0) {
    ...
    if (our_def == 0 && danger > 0) {
       ...
       * nobody behind them. */
      if (mapleai_process_defender_want(ait, nmap, pplayer, pcity, danger, choice,
                                    martial_value)) {
    ...
    if (build_walls) {
      ...
      /* HACK: This needs changing if multiple improvements provide
       * this effect. */
      wall_id = mapleai_find_source_building(pcity, EFT_DEFEND_BONUS, NULL);
    ...
    if (!def_unit_selected) {
      if ((danger > 0 && num_defenders <= urgency) || martial_value > 0) {
        ...
        if (mapleai_process_defender_want(ait, nmap, pplayer, pcity, danger,
  ...
  punittype = mapleai_choose_bodyguard(ait, nmap, pcity, TC_LAND, L_DEFEND_GOOD,
                                   allow_gold_upkeep);
  if (punittype) {
    mapleai_unit_consider_bodyguard(ait, nmap, pcity, punittype, choice);
  }
  ...
  mapleai_choose_diplomat_offensive(ait, nmap, pplayer, pcity, choice);

  /* Consider making a sea bodyguard */
  punittype = mapleai_choose_bodyguard(ait, nmap, pcity, TC_OCEAN, L_DEFEND_GOOD,
                                   allow_gold_upkeep);
  if (punittype) {
    mapleai_unit_consider_bodyguard(ait, nmap, pcity, punittype, choice);
  }
  ...
  (void) mapleai_choose_attacker_air(ait, nmap, pplayer, pcity, choice,
                                 allow_gold_upkeep);

  /* Consider making a paratrooper */
  mapleai_choose_paratrooper(ait, nmap, pplayer, pcity, choice, allow_gold_upkeep);

  /* Check if we want a sailing attacker. Have to put sailing first
     before we mung the seamap */
  punittype = mapleai_choose_attacker(ait, nmap, pcity, TC_OCEAN, allow_gold_upkeep);
  if (punittype) {
   ...
   */
  punittype = mapleai_choose_attacker(ait, nmap, pcity, TC_LAND, allow_gold_upkeep);
  if (punittype) {
  ...
  /* Consider a hunter */
  mapleai_hunter_choice(ait, pplayer, pcity, choice, allow_gold_upkeep);
