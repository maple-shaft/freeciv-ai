...
static void mapleai_diplomacy_new(struct ai_type *ait,
                              const struct player *plr1,
                              const struct player *plr2);
static void mapleai_diplomacy_defaults(struct ai_type *ait,
                                   const struct player *plr1,
                                   const struct player *plr2);
static void mapleai_diplomacy_destroy(struct ai_type *ait,
                                  const struct player *plr1,
                                  const struct player *plr2);
...
void mapleai_data_init(struct ai_type *ait, struct player *pplayer)
{
  ...
  players_iterate(aplayer) {
    ...
    mapleai_diplomacy_new(ait, pplayer, aplayer);
    mapleai_diplomacy_defaults(ait, pplayer, aplayer);
    /* create ai diplomacy state of this player */
    if (aplayer != pplayer) {
      mapleai_diplomacy_new(ait, aplayer, pplayer);
      mapleai_diplomacy_defaults(ait, aplayer, pplayer);
    }
  } players_iterate_end;
...
/************************************************************************//**
  ...
  between mapleai_data_phase_begin() and mapleai_data_phase_finished() calls.
****************************************************************************/
...
****************************************************************************/
void mapleai_data_phase_begin(struct ai_type *ait, struct player *pplayer,
                          bool is_new_phase)
{
  struct ai_plr *ai = def_ai_player_data(pplayer, ait);
  bool caller_closes;
  ...
  if (is_ai(pplayer) && !is_barbarian(pplayer) && is_new_phase) {
    mapleai_diplomacy_begin_new_phase(ait, pplayer);
  }
...
/************************************************************************//**
  ...
  closing.
****************************************************************************/
struct ai_plr *mapleai_plr_data_get(struct ai_type *ait, struct player *pplayer,
                                bool *caller_closes)
{
  struct ai_plr *ai = def_ai_player_data(pplayer, ait);
  ...
  if (ai->last_num_continents != wld.map.num_continents
    ...
    if (ai->phase_initialized) {
      mapleai_data_phase_finished(ait, pplayer);
      mapleai_data_phase_begin(ait, pplayer, FALSE);
    } else {
      /* wrong order */
      log_debug("%s ai data phase closed when mapleai_plr_data_get() called",
                player_name(pplayer));
      mapleai_data_phase_begin(ait, pplayer, FALSE);
      if (caller_closes != NULL) {
        *caller_closes = TRUE;
...
static void mapleai_diplomacy_defaults(struct ai_type *ait,
                                   ...
                                   const struct player *plr2)
{
  struct ai_dip_intel *player_intel = mapleai_diplomacy_get(ait, plr1, plr2);
...
****************************************************************************/
struct ai_dip_intel *mapleai_diplomacy_get(struct ai_type *ait,
                                       const struct player *plr1,
                                       const struct player *plr2)
{
                                  ...
                                  const struct player *plr2)
{
  ...
  if (*player_intel_slot != NULL) {
    free(mapleai_diplomacy_get(ait, plr1, plr2));
  }
  ...
  multipliers_iterate(ppol) {
    if (multiplier_can_be_changed(ppol, pplayer)) {
      ...
      city_list_iterate(pplayer->cities, pcity) {
        orig_value += mapleai_city_want(pplayer, pcity, adv, NULL);
      ...
      if (mp_val > ppol->start) {
        ...
        city_list_iterate(pplayer->cities, pcity) {
          new_value += mapleai_city_want(pplayer, pcity, adv, NULL);
        } city_list_iterate_end;
...
****************************************************************************/
void mapleai_gov_value(struct ai_type *ait, struct player *pplayer,
                   struct government *gov, adv_want *val, bool *override)
{
  ...
  city_list_iterate(pplayer->cities, pcity) {
    ...
    *val += mapleai_city_want(pplayer, pcity, adv, NULL);
    ...
    effect_list_iterate(get_req_source_effects(&source), peffect) {
      ...
      if (active) {
        ...
        v1 = mapleai_effect_value(pplayer, adv, pcity, capital,
                              turns, peffect, 1,
                              nplayers);
